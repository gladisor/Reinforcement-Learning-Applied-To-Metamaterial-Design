from models import ResNetActor, ResNetCritic
import torch
...
class DDPG():
    def __init__(self, params):
        ...
        ## Networks
        self.actor = ResNetActor(params.IN_SIZE, params.ACTOR_N_HIDDEN, params.ACTOR_H_SIZE, params.N_ACTIONS,
                                params.ACTION_RANGE)
        self.targetActor = ResNetActor(params.IN_SIZE, params.ACTOR_N_HIDDEN, params.ACTOR_H_SIZE, params.N_ACTIONS,
                                      params.ACTION_RANGE)
        self.critic = ResNetCritic(params.IN_SIZE, params.CRITIC_N_HIDDEN, params.CRITIC_H_SIZE, params.N_ACTIONS)
        self.targetCritic = ResNetCritic(params.IN_SIZE, params.CRITIC_N_HIDDEN, params.CRITIC_H_SIZE, params.N_ACTIONS)
        ...
        if params.load:
            ...
            self.actor.load_state_dict(actorCheckpoint['state_dict'])
            self.critic.load_state_dict(criticCheckpoint['state_dict'])
    ...
    def select_action(self, img, state):
        with torch.no_grad():
            ...
            action = self.actor(img.cuda(), state.cuda()).cpu() + torch.tensor([noise])
            action.clamp_(-self.actionRange, self.actionRange)
        return action

    def select_action_ou(self, img, state):
        with torch.no_grad():
            action = self.actor(img.cuda(), state.cuda()).cpu() + self.noise()
            action.clamp_(-self.actionRange, self.actionRange)
        return action
