function Q = getMetric_thinShells_radii_material(xM, av, c_pv,rho_shv, k0amax, k0amin, nfreq)
...
% c_p = 5480;
% rho_sh = 8850;
%Titanium: Density~ 4.5 g/cm3, E~ 110 GPa, v=~0.34.

hav = av/10; %thicknesses of  thin shells
% Properties of nickel thin shells 
% c_p_min=4.0e3; c_p_max=6.0e3;
% c_pv = c_p_min + (c_p_max-c_p_min).*rand(M,1);
% 
% rho_sh_min=2.0e3; rho_sh_max=9.0e3;
% rho_shv = rho_sh_min + (rho_sh_max-rho_sh_min).*rand(M,1);
...
% [xM,M]=RandomPointsInAnnulus(R1,R2,aa,M_ring,bb) 
% % M=size(xM0,1)
% x=reshape(xM0',M*2,1); %Initial Position Column Vector
